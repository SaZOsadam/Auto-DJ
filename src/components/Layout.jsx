...
export default function Layout() {
  ...
  // Poll rotation state every second
  useEffect(() => {
    const tick = () => {
      ...
      <aside className={`fixed lg:sticky top-0 left-0 z-40 h-screen w-64 bg-gray-800 border-r border-gray-700 flex flex-col transition-transform duration-200 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="p-4 border-b border-gray-700">
          <Link to="/dashboard" onClick={closeSidebar} className="flex items-center gap-2">
            <span className="text-2xl font-bold text-green-400">AutoDJ</span>
          </Link>
          {rotationActive && (
            <Link to="/rotation" onClick={closeSidebar} className="mt-3 flex items-center gap-2 bg-green-900/40 border border-green-700/50 rounded-lg px-3 py-2">
              <span className="w-2.5 h-2.5 rounded-full bg-green-500 animate-pulse"></span>
              <span className="text-green-400 text-sm font-semibold">Rotation Active</span>
              {timerDisplay && <span className="ml-auto text-green-400 font-mono text-sm">{timerDisplay}</span>}
            </Link>
          )}
        </div>
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {item.to === '/rotation' && rotationActive && !active && (
        ...
        <div className="p-4 border-t border-gray-700">
          <p className="text-gray-500 text-xs text-center">AutoDJ v1.0</p>
        </div>
      </aside>
